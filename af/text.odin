...
internal_font_load_rune :: proc(font: ^DrawableFont, codepoint: rune) -> int {
	...
	glyph_info := GlyphInfo {
		...
		slot = slot_to_free,
		size = Vec2{f32(buff_w) / f32(font.size), f32(buff_h) / f32(font.size)},
		// offset = Vec2{f32(glyph_metric_minx) / f32(font.size), 0}, // -f32(glyph_metric_miny) / f32(font.size),
		offset = Vec2{f32(x0) / f32(font.size), (f32(-y0 - h)) / f32(font.size)}, // -f32(glyph_metric_miny) / f32(font.size),
