...
internal_initialize_text:: proc() {
    if ttf.Init() < 0 {
        debug_log("Couldn't initialize sdl/ttf", severity=.FatalError)
    }
}
...
new_font :: proc(file: cstring, ptsize: int) -> ^DrawableFont {
    size := ptsize
    font := DrawableFont{
        file=file, 
        size=size,
    ...
    font.slot_size = int(math.floor_f32(magic_font_size_multiplier_lmao * f32(ptsize))) + 2 * padding;
    ...
    font.vendor_intermediate_bitmap = sdl2.CreateRGBSurface(
        0, c.int(font.slot_size), c.int(font.slot_size), 32, 0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF
    )
...
internal_font_load_rune :: proc(font: ^DrawableFont, codepoint: rune) -> int { 
    ...
    sdl2_white := sdl2.Color{r=0xFF,g=0xFF,b=0xFF,a=0xFF}
    glyph_metric_minx, glyph_metric_maxx, glyph_metric_miny, glyph_metric_maxy, glyph_metric_advance: c.int
    ttf.GlyphMetrics32(font.vendor_font, codepoint, &glyph_metric_minx, &glyph_metric_maxx, &glyph_metric_miny, &glyph_metric_maxy, &glyph_metric_advance) 
    ...
    insert_x := slot_x * font.slot_size + font.slot_size / 2 - int(surface_buffer_width) / 2 + 
    ...
    when debug_atlas {
        ...
        for i in 0..<int(surface.h) {
            ...
            pixels[int(surface_buffer_width - 1) * 4 + i * int(surface_buffer_width) * 4 + 1] = 0xFF
            pixels[int(surface_buffer_width - 1) * 4 + i * int(surface_buffer_width) * 4 + 2] = 0xFF
            pixels[int(surface_buffer_width - 1) * 4 + i * int(surface_buffer_width) * 4 + 3] = 0xFF
        }
    }
    ...
    glyph_info := GlyphInfo{
        ...
        slot = slot_to_free,
        size = Vec2{
            f32(surface_real_width) / f32(font.size),
            f32(surface.h) / f32(font.size),
        },
        offset = Vec2{
            f32(glyph_metric_minx) / f32(font.size),
            // -f32(glyph_metric_miny) / f32(font.size),
            0
        },
...
utf8_next_rune :: proc(str: string, pos: int) -> (rune, int) {
    ...
    if (
        ...
        return rune(
            int((b1 & (15)) << 12) + (Get__XXXXXX(b2) << 6) + (Get__XXXXXX(b3))), 3
    }
    ...
    if (
        ...
        return rune(int((b1 & (15)) << 18) + (Get__XXXXXX(b2) << 12) + (Get__XXXXXX(b3) << 6) + (Get__XXXXXX(b4))), 4
    }
...
utf8_test :: proc() {
    ...
    for i in 0..<len(testcases) {
        ...
        if codepoint != tt.expected_code_point {
            debug_log("codepoint %v != expected %v for testcase %d", codepoint, tt.expected_code_point, i)
        }
    }
}
